...
- **Global scope**: `/Users/macbbook/DCP/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/macbbook/DCP/.claude/notes/`)
...
2. Glob `/Users/macbbook/DCP/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/macbbook/DCP/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/macbbook/DCP/.claude/notes/` — works in any directory
